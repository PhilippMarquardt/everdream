"""Common runtime and distributed utilities for everdream."""
...
# Override with EVERDREAM_DTYPE env var: "bfloat16", "float16", "float32"
_DTYPE_MAP = {"bfloat16": torch.bfloat16, "float16": torch.float16, "float32": torch.float32}
def _detect_compute_dtype():
    env = os.environ.get("EVERDREAM_DTYPE") or os.environ.get("NANOCHAT_DTYPE")
    if env is not None:
        return _DTYPE_MAP[env], f"set via EVERDREAM_DTYPE={env}"
...
def get_base_dir():
    ...
    if os.environ.get("EVERDREAM_BASE_DIR"):
        nanochat_dir = os.environ.get("EVERDREAM_BASE_DIR")
    elif os.environ.get("NANOCHAT_BASE_DIR"):
        nanochat_dir = os.environ.get("NANOCHAT_BASE_DIR")
    else:
        home_dir = os.path.expanduser("~")
        cache_dir = os.path.join(home_dir, ".cache")
        nanochat_dir = os.path.join(cache_dir, "everdream")
