...
class HuggingFaceTokenizer:
    ...
    def _encode_one(self, text, prepend=None, append=None, num_threads=None):
        ...
        ids.extend(encoded.ids if hasattr(encoded, "ids") else encoded)
...
class RustBPETokenizer:
    ...
    def train_from_iterator(cls, text_iterator, vocab_size):
        if rustbpe is None:
            raise RuntimeError("rustbpe is required to train the default tokenizer. Install rustbpe or use a HuggingFace tokenizer.")
...
def get_token_bytes(device="cpu", tokenizer=None, tokenizer_dir=None, source="local"):
    ...
    if tokenizer is not None:
        ...
        return token_bytes
    if source == "hf":
        tok = get_tokenizer(tokenizer_dir=tokenizer_dir, source=source)
        return get_token_bytes(device=device, tokenizer=tok)
    from everdream.common import get_base_dir
    if tokenizer_dir is None:
        base_dir = get_base_dir()
        tokenizer_dir = os.path.join(base_dir, "tokenizer")
